...
- **Twenty-three Album background treatments are now available for the panels too.** Dusk, Ice, Rose, Paper, Monolith, Lantern, Noir, Velvet, Mirage, Bloom, Cloud, Nocturne, Liquid, Tidal, Vinyl, Corona, Crescent, Grid, Glass veil, Material, Poster, Studio and Spectrum existed only behind the player; they can now be chosen for the Volume, Progress and Quick-actions surfaces as well. They keep the composition they have on the player — where the colour sits and which way it runs — at the lightness a surface with controls on it can carry, since a player background sits over artwork and works by transparency while a panel backdrop *is* the surface.

- **The same background effect is now called the same thing in both lists.** Ocean, Sunset and Prism existed under two names — "Ocean wash" in Album background and "Deep ocean" in Panel background were the same treatment, and so were "Sunset wash" and "Sunrise", and "Prismatic field" and "Prism". They read the same in both pickers now. Nothing was renamed underneath, so saved and published themes are untouched; only the words on screen changed. Separately, the panel's **Split tone** now actually splits: it was a left-to-right ramp, near-identical to Album duotone beside it and nothing like the player's Split tone, and it is now two tones divided by a line the way both lists have always described it.

- **Three background names stopped meaning two different things.** Ember, Horizon and Aurora existed in both the Album background list and the Panel background list, and drew unrelated pictures in each. Player Ember was a single warm glow tucked into a corner; panel Ember was a full diagonal gradient. Player Horizon darkened only the lower band; panel Horizon put a bright band across the middle. Player Aurora was two ribbon glows on black; panel Aurora was a plain linear wash. Picking the same name for the player and the panel now gives you the same treatment, adapted only where the surface demands it — a panel keeps a dark field under the composition, because controls sit on it. Nothing was renamed, so every saved and published theme keeps working; the three panel backdrops simply draw what their names say now.

- **Text can also carry an outline and a backdrop.** Beside the shadow, Text → Title (or Artist) now offers an **Outline** — a stroke around the letters in Hairline, Thin, Medium or Bold, in its own colour — and a **Backdrop**, a plate behind the words in Highlight, Soft or Block with its own colour and opacity. They answer the same problem from opposite ends: over a full-bleed cover an outline separates the letters from whatever is behind them, while a backdrop replaces what is behind them outright. The outline's thickness follows the type rather than being a fixed width, so the same setting reads the same on a large title and a small artist line, and it shrinks with a title that had to shrink. Everything defaults to off, both are set separately for title and artist, and neither reaches the always-on display.

- **The track title and artist can cast a shadow.** Text → Title (or Artist) → **Shadow** offers five shapes — Soft, Hard, Glow, Lift and Subtle — each with its own colour (black, the album's colour, white, or one you pick) and an intensity from 0 to 200%. It exists for the case a full-bleed cover creates: light type over a light patch of artwork, where the only fix used to be dimming the picture. Soft is the one to reach for; Glow sits centred with no offset, so in the album's colour it reads as light coming off the letters rather than as a shadow. Title and artist are set separately, like every other text control. The default is no shadow, so nothing changes until you ask for it, and the always-on display is deliberately untouched — it has its own controls, and a blur is the last thing an always-on panel needs.
...
- **A Community theme can now carry a real photo of its author's watch.** Every preview in the gallery is drawn by the phone from the theme's settings, which is honest but is still a drawing — and the one thing it cannot show is how the theme actually looks on a wrist, at real size, with real album art. When submitting a theme you can now attach **one** screenshot of your own watch's Player screen, picked from your phone's gallery like any other image. It is entirely optional: attach nothing and the theme looks exactly as it does today. Where a theme has one, its detail page shows the photo on the Player instead of the generated preview, labelled so you always know which of the two you are looking at, with a button to switch back to the generated one — and that choice is remembered. Turning it off does not merely hide the photos, it stops the app downloading them at all, so it doubles as the answer for anyone who would rather the gallery fetched nothing but text. Tapping either one still opens it full screen. Every other screen — always-on display, Volume, Progress, Quick panel, Queue — keeps rendering locally as before, and gallery cards stay generated so scrolling the list costs no more than it did. What you attach becomes public permanently, including the cover, track and clock visible in it, and the submission screen says so before the picker opens; a moderator can also approve a theme while refusing its photo.

- **One more icon to pick from, tucked out of the way.** Change icon — the grid you reach when giving a button, gesture or action a look of its own — now also offers the Haibane Renmei mark, but only while **Show archived options** is switched on in Settings. It sits behind that switch for a different reason than the watch layouts do: nothing is wrong with it and nothing was retired, it simply belongs to something other than this app, and putting it in the ordinary grid would present it beside the play and skip glyphs as though it meant something to a media action. It is appended rather than sorted in, so turning the switch on never moves the icons you already know where to find.

- **The colour picker now shows the colour's code, and remembers the ones you used.** Every place you pick a colour — title, artist, clock, shading, the accent floor, the mini buttons — the wheel was the only way in and the only way out: the value it produced was never shown, so it could not be copied, and nothing could be pasted in. The picker now carries the `#RRGGBB` code above the wheel as ordinary editable text, with a button to copy it; typing or pasting a code moves the wheel to it, and a shorthand like `#f00` or an eight-digit value with transparency in front are both understood. Under the wheel sits a row of the colours you last applied — anywhere in the app, not just on the row you are editing — so putting the title's colour on the artist is one tap and never involves the clipboard at all. The list is kept on this phone only: it is a record of what you were working on, not part of a theme, so it is not synced to the watch, not saved into a theme and not carried by a backup.
...
- **Album art is now a composition plus an independent filter layer.** Album art style gains twelve authored treatments — Prismatic field, Crescent rim, Tidal bands, Paper sleeve, Lantern rise, Mirage split, Quiet grid, Nocturne, Cloud study, Liquid pools, Colour monolith and Split tone — while Filter gains Moss, Lavender haze, Cherry, Deep sea, Dust, Bleach, Moonlight, Dream, Infrared, Forest, Silver and Candy alongside its original sixteen photo-editor treatments. The filter is no longer hidden among artwork styles: it can be combined with any composition, is applied as its own colour pass on the player, Compose-owned cover windows, dedicated panel screens and an AOD set to Follow, and renders from the same values in the phone preview. Every choice is face-scoped, saved in themes and accepted by Community-theme import and validation; the old `filter_*` style values remain readable for existing themes.
...
- **Edge seeking now shows where the track actually is, and can be called off.** Holding the bezel to scrub used to overwrite the ring with the finger's position from the first touch, so while choosing a destination there was nothing left on screen saying where the song was — the one reference the decision needs. A marker now sits on the ring at the live position for the lifetime of the drag, in the palette's own tertiary tone so it belongs to the ring it crosses, and it keeps advancing with the track while you decide rather than freezing at the moment you touched down. Changing your mind is also possible now: drag the finger in to the middle of the screen and let go, and the track carries on untouched. Crossing in plays the seek backwards — the arc walks back to the marker and the time counts back with it — while the centre readout gives way to a ✕ in the same palette colour, because a destination time that is about to be discarded is the one thing worth not showing. Sliding back out to the bezel resumes the scrub from wherever the finger is. A gesture the system takes away mid-drag now cancels too, instead of committing a seek to wherever the finger happened to be. Both the player's bezel and the dedicated progress screen behave the same way.
- **Two new dedicated full screens: Volume and Progress.** Alongside the existing rotary/gesture-triggered overlay (unchanged), **Open volume screen** and **Open progress screen** are now assignable to any button, gesture or quick-panel slot like any other action, opening a screen that stays up until dismissed instead of auto-hiding. The Progress action uses a neutral scrubber glyph rather than a misleading timed-skip icon: assigning it opens a destination and does not itself jump the track. Each screen draws *your* panel — the same configured arc or ring, shared backdrop, artwork treatment, shading, album-derived colours and typeface used by the corresponding overlay — and saved themes reach them too. Both meters remain draggable while swiping outside their active band still dismisses the screen, and tapping the centred readout toggles play/pause without adding another visible transport button. Progress now leaves previous/next navigation on the main player and gives the round screen to relative seeking instead: −5, −10 and −30 seconds stand vertically on the left, +5, +10 and +30 on the right, with the common ten-second choices larger. The columns sit closer to the centre without colliding with the time, and their outer buttons follow the glass inward so all six controls remain inside a round display. The playback-speed pill occupies the free space directly above the optically centred time, cycles from 0.5× to 2× in 0.25× steps and wraps, while every skip moves the shared progress ring and time immediately before the phone replies; the later sync still corrects a player that clamps or ignores the command. Volume likewise centres its percentage readout precisely, places quieter/louder along the direction in which the selected arc fills, changes it in exact five-percentage-point steps and reflects each tap locally before phone confirmation. The crown continues to work on both screens.
...
- **Note, Chat and Metadata can reshape their cover — or hide it.** Note draws the track as a small cover beside one line of text, Chat as the sender's avatar in its voice bubble, and Metadata as a small thumbnail beside the credit table; all three were a fixed shape with no way to change it. **Cover shape** now sits on the Player page for each, offering the same silhouette vocabulary Carousel's cards already use plus a new **Samsung style** between Squircle and Circle: rounded, square, squircle, Samsung style or circle. A new **Cover art** switch beside each can also hide the cover altogether, so Note's sentence, Chat's waveform and Metadata's table each reclaim the space it leaves rather than showing a gap. Every one of these is its own per-layout setting like every other appearance choice — it shows in the phone's live preview, saves per theme, and choosing a shape or hiding the cover on one face never touches another.
...
- **Title and artist can be forced to a text case.** **Title case** and **Artist case**, on the Text page, independently choose Normal (whatever the playing app reports), UPPERCASE, lowercase or **Title Case**, which turns `TITULO DE UM TEXTO QUALQUER` into `Titulo De Um Texto Qualquer` by capitalising every word and normalising the remaining letters to lowercase. Whitespace, punctuation and accented letters are preserved. The choice reaches every face and is applied last, so it always wins over a face's own styling (Vinyl's designed all-caps title, for one) without disturbing weight, size, opacity or spacing. Note, whose title and artist share one sentence rather than two separate lines, cases each half by its own setting before joining them, so choosing UPPERCASE for the title there does not also shout the artist. The fourth option is also understood by the phone preview, saved themes and Community-theme import and validation.
- **Discover Community themes.** A shopping-bag button in the toolbar, beside search and help, opens an opt-in gallery of publicly published looks from anywhere in the app; the Watch themes screen opens the same gallery. Search by theme or author, filter by base layout, and order the downloaded public catalogue by newest or most liked — all locally, with no account. Gallery cards and moderation previews stay synthetic. Tapping a card opens a detail page before installation, with local Player, always-on, Volume, Progress, Quick panel, and Queue renders: their text, timing, and queue remain sample data, while the current album cover may appear only from memory on the phone. It is never uploaded, transmitted, captured as a screenshot, or paired with live title, artist, or playback data. Only **Add and apply** puts the theme in **My themes**, where it remains your own local copy.
...
- **The gallery hides themes you already added, and says so.** A new **New to me** chip sits beside the layout and sort filters and ships **on**: the gallery is where you go to find something you do not have, and a theme already sitting in My themes is the one result that cannot be that. Turn it off to see everything again. It reads only the local library, so it costs no network request and no Firebase read, and it never hides a theme the **Liked** filter is asking for — being shown an empty list because you both liked *and* installed something would read as a broken filter. Once every published theme is installed, the empty gallery says exactly that instead of blaming your filters.
- **The watch preview opens full screen.** Tapping the round preview on the **Watch appearance** tab, or the one on a community theme's detail page, enlarges it to fill the screen; tap anywhere to close. It is the same preview redrawn at that size rather than a magnified thumbnail, so text, rings and artwork are rendered at full resolution, and it keeps following whatever it was showing — the surface you were editing, the track that is playing, and the live progress ring. On a theme's detail page it opens on whichever surface the toggle beneath the card has selected.
- **Panel Background gets five more treatments that aren't gradients: Dot grid, Scanline, Radar, Contour lines and Faceted.** Every existing choice in that picker builds on a soft field of colour — a cloud, a glow, a wash — so this adds a different family instead of another entry in the same one: a grid of dots, horizontal scan bands, concentric radar rings, topographic contour lines, and a faceted crystal made of flat-shaded triangles, each derived from the album's own accent colour rather than drawn in a fixed hue. They render identically on the watch, the phone's live preview and the dedicated Volume/Progress screens, are saved per face, and are accepted by Community themes.
...
- **Svartifoss is listed once among the watch's ongoing activities, not twice.** On watches that show ongoing activities as a list — most visibly the Now Bar on Galaxy Watches running One UI 8 Watch — playback put two Svartifoss entries side by side, one with the app icon and the track, one with the equalizer bars, and tapping the bar asked which of the two to open even though both opened the same app. Only one of them was ever the app's own: the watch app also marked its background notification as a *media* notification, and Wear OS turns every media notification into an ongoing activity of its own. Google's guidance is that a watch should post no media notification at all while the music is playing on the phone, which is always the case here, so it no longer does. What remains is the app's own entry, and it opens Svartifoss. The watch's media session is still published; only the notification changed.

- **Customizing the watch on the phone no longer makes the app's memory keep growing.** A report of very high memory use while customizing turned up two real leaks. The live preview built a fresh copy of the Google Sans Flex font on every redraw — several times a frame while it animates — and Android keeps styled variants of each loaded font for the rest of the session, so time spent on the Watch tab kept adding font data that was never released. Fonts are now created once and reused, with a small bounded cache for the Flex axis combinations the preview and the font pickers ask for. The Community themes gallery kept every author photo it had loaded in memory until the list was refreshed, however far you scrolled; it now holds at most a few megabytes of them and reloads the rest from disk when their card comes back.

- **Signing in to Community themes could close the app instead of failing.** On a device whose framework does not carry the Android 14 APIs it reports having — a spoofed API level, a trimmed system image — Credential Manager cannot resolve the platform class it needs, and the failure it raises is an `Error` rather than an exception. It therefore passed straight through the sign-in's error handling and took the process down on a tap of **Sign in and submit**. It is now reported as a failed sign-in like any other, and browsing, liking and installing themes — none of which need an account — go on working.
...
- **Screen readers could not name the playback buttons in the track details sheet.** The previous, play/pause and next buttons there had no spoken label at all, so TalkBack announced three unlabelled buttons; the play/pause one also kept whatever label it was given even after switching between playing and paused. It now says Play or Pause to match the icon actually on screen, the way the mini player already did. The button that changes an action's icon in the action editor was unlabelled too.

- **Small controls were below the size Android asks for.** The mini player's previous/play/next buttons, the same three in the track details sheet, the reorder and overflow buttons on each background layer, the overflow on a saved streaming shortcut and the Remove button on a submitted theme were all between 36 and 46dp, under the 48dp minimum — awkward to hit accurately, and flagged by Android's own accessibility check. All are 48dp now; the icons inside them are drawn at exactly the size they were, so nothing looks bigger, there is just more room around it to land on.

- **Long setting names behaved differently depending on the kind of setting.** A setting with a switch cut its name off after two lines while a setting without one let the name wrap as far as it needed, so in languages with longer words — German, Polish, Finnish — two neighbouring rows in the same list disagreed about whether a name was worth showing in full. Every setting name now wraps in full, and every description that does run out of room ends with an ellipsis instead of stopping mid-word.

- **Two places still showed the default sage green instead of your accent colour.** The tip box at the top of the usage guide kept the built-in green no matter which accent you had picked, or what colour the current album art had produced — the guide opens as its own screen, outside the one that repaints everything when the accent changes, and that box was the only element there nobody had told about it. Separately, the colour picker opened on a very slightly different green from the one the watch preview draws beside it: the neutral watch accent had been typed out by hand in four different places and one of those copies had drifted a shade. All four now read the same single value, so the swatch you start from is exactly the colour the preview is showing.

- **A backup could be refused because of queue covers.** Exporting your settings could fail outright with "Too many shortcutArtwork assets to back up", leaving no backup at all — and nothing on screen explained it, because the shortcuts were never the cause. Covers downloaded for the watch's playback queue were being kept in the same folder as the thumbnails of your saved streaming shortcuts. That folder holds one small file per saved shortcut; the queue adds one per remote cover of every queue ever opened, so a few hundred of them was enough to push it past the limit the backup places on that section, and from then on every export failed — including the parts that had nothing to do with icons. Queue covers now have a cache of their own, in the phone's temporary storage where a re-downloadable file belongs, and it keeps itself to a fixed size instead of growing without end. Whatever has already piled up is cleared on the next launch, so a backup that fails today simply works again. The other half of the same mix-up is gone too: editing your saved shortcuts used to wipe the entire queue-cover cache, and the next queue you opened had to fetch every cover a second time.

- **Fifty-seven typefaces no longer sit visibly high or low inside pills, chips and every other centred container** — Special Elite, Playfair Display, Courier Prime, Bebas Neue, Lato, the IBM Plex family, the Noto family and 50 more. Text is centred by its line box everywhere — Compose measures it, a wrapped `TextView` *is* it, the phone preview computes a baseline from it — so the glyphs land wherever a font puts them inside that box, and each of these shipped with that box off-centre from its own ink by as much as 0.14 em (Special Elite, the most visible case, reported directly). Every one now centres on its own flat capital letters, which is where the well-behaved faces in the catalogue already sit; each font's `licenses/<name>/MODIFICATIONS.txt` records the exact correction, including six faces whose old-style numerals dip below the baseline by design and were deliberately left doing so rather than "corrected" into a different typeface. Fifty of the fifty-seven keep their exact original line height, spacing, wrapping and reserved height — only where the ink sits inside that box moved; seven had virtually no room below their own ordinary lowercase descenders to begin with, so their box was grown by the smallest amount that both centres the text and keeps clearing those letters. Three further bundled fonts were measured and deliberately left alone — a script face whose apparent problem turned out to be a decorative capital, a pixel font whose fix would need an untested metrics value, and a handwriting face whose capitals disagree with each other by design — each recorded in `BundledFontMetricsTest` so the reasoning isn't lost to the next pass.

- **The Graphite, Prism and Liquid Glass panel backgrounds no longer show a broken ring of light around the edge of the watch.** Their bright rim was drawn as a rectangular border matching the square area the app lays out into, and on a round display that rectangle's four edges sit right where the glass actually is while its corners sit beyond it, under the bezel — so the rim rendered as four disconnected bright segments at the top, bottom, left and right instead of one continuous edge. It now traces a circle on a round watch, matching the phone preview, which never had this bug. A square watch is unaffected.

- **"Overlay blur radius" no longer offers a control that does nothing for most Panel backgrounds.** Only about a quarter of the ~35 Panel Background treatments are translucent enough to show the blurred cover through them; the rest are solid fields or fully opaque gradients this radius has no effect on. The row now appears only when the selected background actually uses it, and searching for it while a non-blurred background is active leads to the background picker instead of a control that would visibly do nothing.

- **A release build's watch permanently showed "ERROR" with no album art, artist or title, even though it was connected and tapping the screen still paused the track.** Nothing was wrong with the connection: the watch could no longer decode the `MusicState` payload the phone sends for every track and playback change, so it fell into a generic failure state on the very first message and stayed there, while unrelated commands that never go through that payload (like a tap toggling play/pause) kept working. The cause was in R8's code shrinking, which release builds alone apply: it saw a couple of fields on the protobuf message that no code in that particular app ever *reads* directly and quietly deleted them as unused. That's normally a safe optimization, but this message format resolves its own fields by name through reflection at the moment it's first parsed, and a name that used to exist and suddenly doesn't makes that resolution — and therefore every field in the message, not only the deleted one — fail at once. Both apps now tell R8 to leave every field of every message format alone, matching what the protobuf library itself should have supplied but doesn't for the version in use.
- **The Background page's own name appeared a second time as a stray heading with nothing under it.** The Background editor is one of five pages that reuses a shared name for both a section header and its Preference-storage category, and it was the only one of the five where that internal category kept a visible title — everywhere else that title is intentionally hidden, since the page itself already says what it's for. It's now hidden here too.
- **Exporting a backup with everything selected could fail outright with no explanation.** Two of the ten sections — Icons and Auxiliary data — back up whatever is actually sitting in the app's own storage folders rather than a fixed set of preferences, and a single stray file there (a zero-byte one from an interrupted write, or one larger than the export expected) aborted the *entire* backup with a generic "Couldn't export backup", including the eight unrelated sections that had nothing wrong with them. Such a file is now skipped rather than treated as a reason to fail, and if something still can't be exported, the message now says why instead of leaving it a mystery.
- **"Select all" and "Clear all" on the backup screen no longer show a fixed green that ignores the rest of the app's palette.** Every other accent on that screen — the checkboxes, the Export button — already follows the app's resolved colour (static, custom-picked, or drawn from album art); these two buttons were reading a hardcoded fallback tone instead. They now resolve the same colour as everything else on the screen.

- **Configured swipe gestures work across the whole surface of every player layout.** Compose layouts sat above the shared gesture layer, so a swipe starting on plain artwork worked while the same motion starting over a cover, control or other layout-owned region was swallowed. A dispatch-level host now observes streams claimed by Compose's internal input child without consuming them — including the centre region beneath Chat's conversation bubbles — while rejected streams still fall through to the classic gesture layer exactly once. Up, down and left actions therefore start across the layout while taps, long presses, seeking and visible controls retain their own behaviour. Swiping no longer paints a line along the finger's path. Rightward swipe remains reserved for Wear OS dismiss.
- **A refused submission now says which setting is at fault.** "This saved theme cannot be submitted" named neither the control nor the value, and since a theme is validated as a whole, one option outside the gallery's vocabulary refused everything — with a hundred and forty-seven settings and no clue which to look at. It now reads "The gallery can't accept “Lobster” for Title font", using the row's own name and the label its picker shows, both in the app's language. It still happens for a retired option, which is the case that remains: those stay out of the gallery on purpose, and this at least says so.
- **A saved theme could be refused for using one of the fonts the app offers.** Submitting re-checks a theme against the published contract that says which values each appearance setting may hold, and that contract had never been extended when the font catalogue was: fifty-one of the eighty-seven typefaces the picker offers were missing from it, as were the Ribbon and Frame always-on styles. A theme using any of them was refused outright with "This saved theme cannot be submitted", which names neither the setting nor the value at fault. Because a typeface is chosen per layout, this split by layout rather than by theme — whichever layouts you had set one of those fonts on stopped accepting submissions while the rest kept working, which looked like certain layouts being unsubmittable in themselves. Every font the picker offers and both new always-on styles are part of the contract now, and a new check compares each picker against it on every build, so an option cannot reach the app without reaching the contract as well.
