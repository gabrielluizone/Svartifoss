...
- **The Text page no longer shows "Clock" twice.** The clock's typeface and letterforms were one group and its color and opacity another, with Title, Artist and Icon in between — so the page listed Clock at the top and again at the bottom, and neither half was the whole thing. They are now a single Clock group, after Artist, with everything about the clock in it.
- **The artist's color moved to where the artist's other settings are.** It sat on the Colors page while every other artist setting was on Text. Colors keeps the album palette and the settings for surfaces that aren't text — progress ring, volume, quick panel.
...
- **New section: Clock typography.** The clock on the now-playing screen was locked to the track font with no controls of its own. Watch face → Text now has a Clock block with its own **typeface**, **weight**, **italic**, **size** and **letter spacing**, matching what the title and artist already had. The typeface defaults to **Follow track font**, and every other control defaults to exactly what the clock looked like before, so nothing changes until you move something. Saved per face, like the rest of the clock settings. (Its opacity stays where it was, under Style → Clock — one opacity setting, not two.)
...
- **New setting: Title text color.** The track title could never be recolored — every face drew it in its own white. It now takes the same color treatments the artist line has (album palette, a harmony, or a color you pick), in Watch face → Text → Title, with the same optional contrast correction. The default is **Face default (as designed)**, which keeps each face exactly as it draws its title today, so nothing changes until you pick something. Each face's own opacity is preserved either way — the ones that sit their title slightly transparent against a bright cover stay that way, only the hue changes.
- **New setting: Adapt clock color for contrast.** The same correction the artist line already offers, for the clock: when its color is set to Album color and that color sits too close to the artwork behind it, the clock is brightened or darkened while keeping the album's hue. Off by default, in Watch face → Text → Clock. Only applies to Album color — White and a custom color are choices you made, and Dynamic already resolves legibility its own way.
