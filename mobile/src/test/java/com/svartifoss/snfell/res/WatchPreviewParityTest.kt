...
class WatchPreviewParityTest {
    ...
    /**
     * The clock and the four quadrant hints are host chrome on the watch, drawn above every face,
     * and the miniature has to place them from the same answer.
     *
     * Both halves of this failed before, in opposite directions: the preview drew four hints for
     * Classic and two for every Compose face, while the watch drew none at all on fourteen of the
     * fifteen. What kept that invisible is that each renderer looked internally consistent - the
     * only way to catch it is to require that neither computes the placement itself.
     */
    @Test
    fun playerChromeIsPlacedByTheSharedResolverOnBothSides() {
        val preview = previewSource
        val host = repoFile(
                "wear/src/main/java/com/svartifoss/snfell/watch/view/MainActivity.kt").readText()
        val curvedClock = repoFile(
                "wear/src/main/java/com/svartifoss/snfell/watch/view/CurvedClockView.kt").readText()

        for ((renderer, source) in listOf("The watch host" to host, "The phone preview" to preview)) {
            assertTrue("$renderer must place the chrome through PlayerChromeLayout.resolve",
                    source.contains("PlayerChromeLayout.resolve("))
        }
        for (shared in listOf("curvedClockBaselineRadiusDp", "curvedClockSweepDegrees",
                "curvedClockStartAngleDegrees")) {
            assertTrue("CurvedClockView must take $shared from PlayerChromeLayout",
                    curvedClock.contains("PlayerChromeLayout.$shared("))
            assertTrue("The preview's curved clock must take $shared from PlayerChromeLayout",
                    preview.contains("PlayerChromeLayout.$shared("))
        }
        // Each of these bought room by making a piece of chrome smaller, which is the answer this
        // replaced rather than refined. They must not come back on either side.
        for (removed in listOf("topHintOffset", "fitClassicClockBesideHint", "reserveHint")) {
            assertTrue("$removed shrinks the chrome to make room; it must stay removed",
                    !host.contains(removed) && !preview.contains(removed))
        }
    }

    /**
     * Hiding the player controls has to remove them from the *layout*, on both renderers.
     *
     * It shipped as `alpha = 0`, which left the band reserved and something round still drawn where
     * the button had been - so the switch read as doing nothing, which is exactly what was
     * reported. The two centred-transport faces are the ones with a band to give back, and the
     * miniature has to give it back at the same moment or it goes on showing a player the watch no
     * longer draws.
     */
    @Test
    fun hidingTheControlsRemovesTheTransportFromBothLayouts() {
        val preview = previewSource
        val expressive = repoFile(
                "wear/src/main/java/com/svartifoss/snfell/watch/view/face/ExpressiveFace.kt")
                .readText()
        val curated = repoFile(
                "wear/src/main/java/com/svartifoss/snfell/watch/view/face/CuratedPlayerFaces.kt")
                .readText()

        for ((face, source) in listOf("Expressive" to expressive, "Material" to curated)) {
            assertTrue("$face must tell transportLayout the transport is gone, not draw it at alpha 0",
                    source.contains("showTransport = state.showControls"))
            assertTrue("$face must keep a centre gesture when it draws no control",
                    source.contains("PlayerControlGeometry.HIDDEN_TRANSPORT_REGION_FRACTION"))
        }
        assertTrue("The preview must make the same allocation from its own controls setting",
                preview.contains("showTransport = playerControlsVisible"))
        for (gate in listOf("if (expressiveControlsVisible) {", "if (controlsVisible) {")) {
            assertTrue("The preview must skip the transport drawing itself, not scale it to nothing",
                    preview.contains(gate))
        }
    }
