...
object UpdateChecker {
    ...
    /**
     * Segment-wise version comparison with pre-release precedence ("2.1.1" < "2.2",
     * "3.1-beta1" < "3.1-beta2" < "3.1"). Tags may carry a leading "v".
     *
     * The pre-release half is not cosmetic and must not be discarded: this project ships betas as
     * `3.1-beta1` / `3.1-beta2`, so a comparison that reads only the numeric part sees both as
     * "3.1" and reports every beta as up to date - which stranded pre-release users on the build
     * they happened to install, both for the next beta *and* for the eventual final release.
     *
